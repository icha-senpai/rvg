...
const user = computed(() => page.props.auth?.user ?? page.props.authUser ?? null)
...
const canManageOperation = computed(() => {
  ...
  const squadronId = operation?.squadron?.id
...
const startConfirmMessage = computed(() => {
  return operation?.title
    ? `You are about to mark "${operation.title}" as In Progress.`
...
watch(
  ...
  (participant) => {
    if (participant) {
      joinForm.roleId = roleIdForParticipant(participant)
...
function confirmStartOperation({ close, finish }) {
  ...
  router.post(route('operations.start', operation.id), {}, {
    preserveScroll: true,
    onSuccess: () => {
      close()
    },
...
function confirmCancelOperation({ close, finish, text }) {
  ...
  router.post(route('operations.cancel', operation.id), {
    reason: text,
  }, {
    preserveScroll: true,
    onSuccess: () => {
        ...
        <div class="relative p-5 md:p-6">
          ...
          <main class="min-w-0 divide-y divide-white/[0.055]">
            <div class="min-w-0 pb-6">
              ...
              <h1 class="mt-3 text-3xl font-black tracking-tight text-horizon-white md:text-4xl">
                {{ displayTitle }}
              </h1>
