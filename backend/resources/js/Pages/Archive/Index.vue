        ...
        <div class="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_24rem] lg:items-end">
          <div class="min-w-0">
            <div class="text-xs font-bold uppercase tracking-[0.28em] text-[color:var(--horizon-text-secondary)]">
              Horizon Internal Knowledge Library
            </div>
          ...
          <form class="rounded-2xl border border-white/10 bg-black/20 p-3 shadow-inner shadow-black/20" @submit.prevent="applySearch">
            <label class="block text-xs font-bold uppercase tracking-[0.2em] text-text-muted" for="archive-search">
              Search Archive
            </label>
            ...
            <div class="mt-2 flex gap-2">
              <input
                ...
                class="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/[0.06] px-3 py-2 text-sm text-horizon-white outline-none transition placeholder:text-text-muted focus:border-[color:var(--horizon-sunset-blue)]/60"
              />

              <button
                type="submit"
                class="rounded-xl border border-[color:var(--horizon-sunset-blue)]/40 bg-[color:var(--horizon-sunset-blue)]/15 px-4 py-2 text-sm font-bold text-horizon-white transition hover:bg-[color:var(--horizon-sunset-blue)]/25"
              >
                Search
              </button>
            </div>
            ...
            <button
              v-if="hasSearch"
              type="button"
              class="mt-2 text-xs font-semibold text-text-secondary transition hover:text-horizon-white"
              @click="clearSearch"
            >
              Clear search
            </button>
      ...
      <section v-if="hasSearch" class="space-y-4">
        ...
        <div v-if="entries.length" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <Link
            ...
            :key="entry.id"
            :href="entry.href"
            class="group rounded-2xl border border-white/10 bg-white/[0.035] p-5 transition hover:-translate-y-0.5 hover:border-[color:var(--horizon-sunset-blue)]/45 hover:bg-white/[0.055]"
          >
            <div class="text-xs font-bold uppercase tracking-[0.2em] text-[color:var(--horizon-sunset-blue)]">
              {{ entry.topic?.title ?? 'Archive Entry' }}
            </div>
            <h3 class="mt-2 text-lg font-black text-horizon-white group-hover:text-[color:var(--horizon-sunset-blue)]">
              {{ entry.title }}
            </h3>
            <p class="mt-2 line-clamp-3 text-sm text-text-secondary">
        ...
        <div v-else class="rounded-2xl border border-white/10 bg-white/[0.035] p-6 text-sm text-text-secondary">
          No visible archive entries matched this search.
        </div>
      </section>
      ...
      <section class="space-y-4">
        ...
        <div v-if="topics.length" class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          <Link
            ...
            :key="topic.id"
            :href="topic.href"
            class="group relative overflow-hidden rounded-[1.75rem] border border-white/10 bg-[linear-gradient(145deg,rgba(255,255,255,0.06),rgba(255,255,255,0.025))] p-5 shadow-[0_18px_55px_rgba(0,0,0,0.22)] transition hover:-translate-y-1 hover:border-[color:var(--horizon-sunset-magenta)]/45"
          >
            ...
            <div class="relative space-y-4">
              ...
              <div>
                <div class="flex flex-wrap gap-2">
                  <span class="rounded-full border border-[color:var(--horizon-sunset-magenta)]/30 bg-[color:var(--horizon-sunset-magenta)]/10 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-[color:var(--horizon-text-primary)]">
                    {{ topic.category_label || 'Archive' }}
                  </span>
                  <span class="rounded-full border border-white/10 bg-white/[0.035] px-3 py-1 text-xs font-semibold text-text-muted">
                    {{ topic.visible_entries_count }} entries
                  </span>
                </div>

                <h3 class="mt-3 text-xl font-black text-horizon-white group-hover:text-[color:var(--horizon-sunset-magenta)]">
                  {{ topic.title }}
                </h3>

                <p class="mt-2 line-clamp-3 text-sm leading-6 text-text-secondary">
              ...
              <div class="flex items-center justify-between border-t border-white/10 pt-4 text-xs font-semibold text-text-muted">
                <span>{{ topic.minimum_rank_label }}</span>
                <span class="text-[color:var(--horizon-sunset-blue)] group-hover:text-horizon-white">Open →</span>
              </div>
            </div>
          </Link>
        </div>
        ...
        <div v-else class="rounded-2xl border border-white/10 bg-white/[0.035] p-6 text-sm text-text-secondary">
          No archive topics are visible for your current access level.
        </div>
      </section>
    </div>
  </HorizonContainer>
