...
class Advanced_Gmap extends Module_Base
{
    ...
    {
        return ['advanced', 'gmap', 'location', 'google map'];
    }
    ...
    {
        if ($this->ep_is_edit_mode()) {
            return ['gmap-api', 'ep-scripts'];
        } else {
            return ['gmap-api', 'ep-advanced-gmap'];
        }
    }
    ...
    protected function register_controls()
    {
        ...
        $repeater->add_control(
            ...
            [
                ...
                'raw' => '<button type="button" class="elementor-button elementor-button-success ep-location-search-btn" title=" '. esc_html__('Search for a location by address and select it to automatically fill the latitude and longitude fields.', 'bdthemes-element-pack') .' ">
                    <i class="eicon-search" style="margin-right:5px;"></i> ' . esc_html__('Search Location', 'bdthemes-element-pack') . '
                </button>
                <div class="ep-location-search-modal" style="display:none; position:fixed; z-index:99999; left:0; top:0; width:100%; height:100%; overflow:auto; background-color:rgba(0,0,0,0.6);">
                    <div class="ep-modal-content" style="background-color:#fefefe; margin:5% auto; padding:20px; border:1px solid #888; width:80%; max-width:600px; border-radius:5px;">
                        <div class="ep-modal-header" style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #eee; padding-bottom:10px; margin-bottom:20px;">
                            <h3 style="margin:0;">' . esc_html__('Search Location', 'bdthemes-element-pack') . '</h3>
                            <button type="button" class="ep-modal-close" style="background:none; border:none; cursor:pointer; font-size:20px;"><i class="eicon-close"></i></button>
                        </div>
                        <div class="ep-modal-body">
                            <div class="ep-search-wrapper" style="display:flex; margin-bottom:15px;">
                                <input type="text" class="ep-address-search" style="flex:1; padding:8px; margin-right:10px; border:1px solid #ddd; border-radius:4px;" placeholder="' . esc_html__('Enter address to search', 'bdthemes-element-pack') . '">
                                <button type="button" class="elementor-button elementor-button-success ep-search-address-button" style="white-space:nowrap;">' . esc_html__('Search', 'bdthemes-element-pack') . '</button>
                            </div>
                            <div class="ep-search-results"></div>
                        </div>
                        <div class="ep-modal-footer" style="border-top:1px solid #eee; padding-top:15px; margin-top:20px; text-align:right;">
                            <button type="button" class="elementor-button elementor-button-success ep-select-location-button" style="display:none;">' . esc_html__('Select Location', 'bdthemes-element-pack') . '</button>
                        </div>
                    </div>
                </div>',
                'label_block' => true,
    ...
    public function render()
    {
        ...
        foreach ($settings['marker'] as $marker_item) {
            ...
            $marker_settings['content'] = isset($marker_item['marker_content']) ? nl2br( wp_kses( $marker_item['marker_content'], element_pack_allow_tags('text') ) ) : '';
		...
		<?php if (empty($ep_api_settings['google_map_key'])): ?>
			<div class="bdt-alert-warning" data-bdt-alert>
                ...
                echo '<p>';
                echo sprintf(esc_html__('Please add your Google Map API key in Element Pack settings. %1$sClick here%2$s to add your Google Map API key.', 'bdthemes-element-pack'), '<a href="' . esc_url($ep_setting_url) . '">', '</a>');
                echo '</p>';
                ?>
...
foreach ($settings['marker'] as $marker_item) {
            ...
            $this->add_render_attribute('bdt-gmap-list-item', 'data-settings', [
                wp_json_encode(array_filter([
                    ...
                    'image' => $image_src,
                    'content' => isset($marker_item['marker_content']) ? nl2br( wp_kses( $marker_item['marker_content'], element_pack_allow_tags('text') ) ) : '',
...
if ((!$image_src) && ($marker_item['custom_marker']['url'] === '')) {
                printf('<img src="%s"/>', '' . esc_url(BDTEP_ASSETS_URL) . 'images/location.svg');
            } else if (!$image_src) {
                printf('<img src="%s"/>', esc_url($marker_item['custom_marker']['url']));
            } else {
                printf('<img src="%s"/>', esc_url($image_src[0]));
            }
            ?>
