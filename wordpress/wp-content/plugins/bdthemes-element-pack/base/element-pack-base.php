...
if (!class_exists("Element_Pack_Base")) {
    class Element_Pack_Base
    {
        ...
        public function theme_force_update()
        {
            ...
            echo wp_kses_post('<h1>' . __("Update Checking..", "bdthemes-element-pack") . '</h1>');
        ...
        {
            // Always use the current site's URL for license validation
            // This allows each subsite to have its own independent license
        ...
        /**
         * Check if this is a multisite installation and if license supports it
         * 
         * @return array
         */
        public static function get_multisite_info()
        {
            ...
            if ($info['is_multisite']) {
                // Get main site URL
                if (function_exists("network_site_url")) {
                    $info['main_site_url'] = network_site_url();
                } elseif (function_exists("network_home_url")) {
                    $info['main_site_url'] = network_home_url();
                }
                
                // Get current site URL
                if (function_exists("site_url")) {
                    $info['current_site_url'] = site_url();
                }
                
                // License uses current site URL for validation (each subsite has its own license)
                $info['license_domain'] = $info['current_site_url'];
            } else {
                // Single site
                if (function_exists("site_url")) {
                    $info['current_site_url'] = site_url();
            ...
            if (!empty($old_respons->is_valid)) {
                if (!empty($old_respons->license_key)) {
                    ...
                    if (empty($response->code)) {
                        if (!empty($response->status) || $response->msg == 'Invalid license code') {
                    ...
                    if (!empty($response->status)) {
                        if (!empty($response->data)) {
                            ...
                            if ($license_obj->is_valid) {
                                ...
                                // Save other_param if present for white label checking
                                if (!empty($license_obj->other_param)) {
                                    $response_obj->other_param = $license_obj->other_param;
                                }
